...
export function prepareFilterBy(opts, result) {
    ...
    opts.filterBy.unshift((image) => {
        return fs.statAsync(image.path).catch(() => {
...
export function extractImages(root, opts, result) {
    ...
    root.walkRules((rule) => {
        const styleFilePath =
            opts.relativeTo === RELATIVE_TO_RULE ?
...
export function runSpritesmith(opts, images) {
    ...
    return new Promise((resolve, reject) => {
        const promises = _.chain(images)
            ...
            .map((images, tmp) => {
                const factory =
                    tmp.indexOf(TYPE_VECTOR) > -1 ? VectorFactory : RasterFactory;

                return factory(opts, images).then((spritesheet) => {
...
export function saveSpritesheets(opts, images, spritesheets) {
    ...
    return Promise.each(spritesheets, (spritesheet) => {
        return (
            ...
            if (!res) {
                throw new Error(
                    "postcss-sprites: Spritesheet requires a relative path."
                );
            }
